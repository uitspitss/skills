...
1. **ツール類のみ適用（推奨）** — 手順 3〜7 と 13 を飛ばし、手順 8〜12・14〜21（oxlint/oxfmt, Vitest, knip, lefthook, worktrunk, CI, .gitignore, CLAUDE.md, README）だけを適用する。既存のフレームワークはそのまま
...
プロジェクトの開発ルールを Claude Code に伝えるための `CLAUDE.md` を生成する:
